...
var wrapGroup = function (group, parentSetUp, parentTearDown) {
    ...
    for (var i=0; i<keys.length; i++) {
        ...
        if (typeof group[k] === 'function') {
            var completeSetUp = getSerialCallback(parentSetUp, setUp);
            var completeTearDown = getSerialCallback(tearDown, parentTearDown);
            tests[k] = wrapTest(completeSetUp, completeTearDown, group[k]);
        }
