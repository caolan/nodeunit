...
function setupTestName(name, key) {
    name = name ? [].concat(name, key) : [key];
...
function isTestspecInName(testspec, name) {
...
function shouldTryToStartModule(opt, name) {
    if (!opt.testspec || isTestspecInName(opt.testspec, name)) {
        return true;
    } else {
        return false;
    }
...
exports.runSuite = function (name, suite, opt, callback) {
    if (!suite) { // I don't understand why I need to check this..
    ...
    async.series({
        runBeforeAll: function (cb) {
            var context = {};
            if (suite.beforeAll) {
                suite.beforeAll.call(context, function (err) {
                    if (err) {
                        cb(err);
                    } else {
                        delete suite.beforeAll;
                        cb(null);
                    }
                });
            } else {
        ...
        assertions: function (cb) {
            async.concatSeries(keys, function (key, cb) {
                var prop = suite[key];
                var _name = setupTestName(name, key);

                if (typeof prop === 'function') {
                    if (shouldTryToStartModule(opt, _name)) {
                        if (opt.moduleStart) {
                            opt.moduleStart();
                        }
                        exports.runTest(_name, suite[key], opt, cb);
                    } else {
                        return cb();
                    }
                } else {
                    exports.runSuite(_name, suite[key], opt, cb);
                }
            }, cb);
        },

        runAfterAll: function (cb) {
            var context = {};
            if (afterAll) {
                afterAll.call(context, function (err) {
                    cb(err);
                });
            } else {
                cb(null);
    ...
    function(err, result) {
        if (!err && result) {
            callback(null, result.assertions);
        } else {
            callback(err);
        }
    });
...
/**
 * Returns a serial callback from an array of functions.
 *
 * @param {Array} fns Array of callback functions.
 * @api private
 */
