...
exports.testCase = function (suite) {
    ...
    return keys.reduce(function (tests, k) {
        tests[k] = function (test) {
            ...
            var run = function(){
                if (tearDown) {
                    ...
                    test.done = function (err) {
                        var tearDownEmitter = new events.EventEmitter();
                        tearDownEmitter.on('done', function(){ done(err);});
                        tearDown.call(context, {done: function(){ tearDownEmitter.emit('done');}});
                    };
                } 
            ...
            if (setUp) {
                var setUpEmitter = new events.EventEmitter();
                setUpEmitter.on('done', run);
                setUp.call(context, {done: function(){ setUpEmitter.emit('done');}});
            } else {
                run();
            }
        };
