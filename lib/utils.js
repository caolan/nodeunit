...
/**
 * Provides a CLI testing environment.
 *
 * Example:
 *
 *  var cliutil = require('nodeunit').utils.cliutil
 ...
 * @api public
 */
exports.cliutil = function (file, args, expected, test) {
    ...
    childProcess.execFile(file, args, function (error, stdout, stderr) {
        ...
        if (expected.hasOwnProperty('stdout')) {
            if (expected.stdout instanceof RegExp) {
                test.ok(stdout.replace(node_deprecated_warning, '').match(expected.stdout));
            } else {
        ...
        if (expected.hasOwnProperty('stderr')) {
            if (expected.stderr instanceof RegExp) {
                test.ok(stderr.replace(node_deprecated_warning, '').match(expected.stderr));
            } else {
